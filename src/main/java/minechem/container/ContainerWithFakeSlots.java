...
public class ContainerWithFakeSlots extends Container {
    ...
    public void clearSlotsDragged() {
        slotsDragged.clear();
    }

    public void addToSlotsDragged(Slot slot) {
        slotsDragged.add(slot);
    }
    ...
    public ItemStack superSlotClick(int slotNum, int mouseButton, int isShiftPressed, EntityPlayer entityPlayer)
    {
        ...
        if (isShiftPressed == 5)
        {
            int l = this.getField_94536_g(this);
            this.setField_94536_g(this, func_94532_c(mouseButton));

            if ((l != 1 || this.getField_94536_g(this) != 2) && l != this.getField_94536_g(this))
            {
                this.func_94533_d();
            } else if (inventoryplayer.getItemStack() == null)
            {
                this.func_94533_d();
            } else if (this.getField_94536_g(this) == 0)
            {
                this.setField_94535_f(this, func_94529_b(mouseButton));

                if (func_94528_d(this.getField_94535_f(this)))
                {
                    this.setField_94536_g(this,1);
                    clearSlotsDragged();
                } else
                {
                    this.func_94533_d();
                }
            } else if (this.getField_94536_g(this) == 1)
            {
                Slot slot = (Slot) this.inventorySlots.get(slotNum);

                if (slot != null && func_94527_a(slot, inventoryplayer.getItemStack(), true) && slot.isItemValid(inventoryplayer.getItemStack()) && inventoryplayer.getItemStack().stackSize > getSlotsDragged().size() && this.canDragIntoSlot(slot))
                {
                    addToSlotsDragged(slot);
                }
            } else if (this.getField_94536_g(this) == 2)
            {
                if (!getSlotsDragged().isEmpty())
                {
                    itemstack3 = inventoryplayer.getItemStack().copy();
                    i1 = inventoryplayer.getItemStack().stackSize;
                    Iterator iterator = getSlotsDragged().iterator();

                    while (iterator.hasNext())
                    {
                        Slot slot1 = (Slot) iterator.next();

                        if (slot1 != null && func_94527_a(slot1, inventoryplayer.getItemStack(), true) && slot1.isItemValid(inventoryplayer.getItemStack()) && inventoryplayer.getItemStack().stackSize >= getSlotsDragged().size() && this.canDragIntoSlot(slot1))
                        {
                            ItemStack itemstack1 = itemstack3.copy();
                            int j1 = slot1.getHasStack() ? slot1.getStack().stackSize : 0;
                            func_94525_a((Set) getSlotsDragged(), this.getField_94535_f(this), itemstack1, j1);
